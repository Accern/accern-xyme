...
from .util import ByteResponse as ByteResponse, FILE_UPLOAD_CHUNK_SIZE as FILE_UPLOAD_CHUNK_SIZE, ServerSideError as ServerSideError, async_compute as async_compute, content_to_csv_bytes as content_to_csv_bytes, df_to_csv_bytes as df_to_csv_bytes, get_age as get_age, get_file_hash as get_file_hash, get_file_upload_chunk_size as get_file_upload_chunk_size, get_max_retry as get_max_retry, get_progress_bar as get_progress_bar, get_retry_sleep as get_retry_sleep, has_graph_easy as has_graph_easy, interpret_ctype as interpret_ctype, is_jupyter as is_jupyter, maybe_json_loads as maybe_json_loads, merge_ctype as merge_ctype, safe_opt_num as safe_opt_num, to_bool as to_bool
...
class CSVBlobHandle(BlobHandle):
    def finish_csv_upload(self, expected_size: int, filename: Optional[str] = ...) -> UploadFilesResponse: ...
    def add_from_file(self, filename: str, progress_bar: Optional[IO[Any]] = ..., requeue_on_finish: Optional[NodeHandle] = ...) -> Optional[UploadFilesResponse]: ...
    def add_from_df(self, df: pd.DataFrame, progress_bar: Optional[IO[Any]] = ..., requeue_on_finish: Optional[NodeHandle] = ...) -> Optional[UploadFilesResponse]: ...
    def add_from_content(self, content: Union[bytes, str, pd.DataFrame], progress_bar: Optional[IO[Any]] = ..., requeue_on_finish: Optional[NodeHandle] = ...) -> Optional[UploadFilesResponse]: ...

class TorchBlobHandle(BlobHandle):
    def finish_torch_upload(self, expected_size: int, filename: Optional[str] = ...) -> UploadFilesResponse: ...
